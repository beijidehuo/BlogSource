<template>
    <article>
        ...
        <el-table ref="multipleTable" :data="blogList" tooltip-effect="dark" stripe border>
            ...
            <el-table-column show-overflow-tooltip v-if="!item.hidden && !item.tags" v-for="(item, index) in headerOptions" :key="index"
            ...
            <el-table-column show-overflow-tooltip v-else-if="item.tags" :key="index" :label="item.label" :prop="item.prop"
        ...
        <el-pagination class="pagination" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="pageindex"
            :page-sizes="size_scoped" :page-size="pagesize" layout="total, sizes, prev, pager, next, jumper" :total="blogTotal">
        </el-pagination>
    ...
    export default {
        ...
        methods: {
            ...
            async getBlogList() {
                this.loading = true;
                try {
                    const res = await Blog.getBlogList({
            ...
            del(scope) {
                this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
                    ...
                    try {
                        await Blog.delBlog(scope.row._id)
