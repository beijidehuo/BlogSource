...
export default {
    components: {
        ...
        Issue,
        LeaveMessage,
        RaindropCanvas: resolve =>
            require(['components/RaindropCanvas/index'], resolve)
    },
    data() {
        return {
            // 刷新
        ...
        getMessages() {
            return new Promise(async (resolve, reject) => {
                ...
                this.messages = res.data.map(item =>
                    this.formatStyleByMessage(item)
                )
                this.$nextTick(() => {})
                resolve()
            })
